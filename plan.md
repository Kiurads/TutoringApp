...
**Tour rewritten again: full sidebar walkthrough, not a 5-item highlight reel.** Feedback after the previous pass was still "not great" — spotlighting only 3-4 dashboard elements per role left most of the sidebar unexplained. `buildSteps()` in `welcome-tour-modal.tsx` now walks every sidebar nav item top to bottom, one step each, with a specific one-line explanation of what that page is for (11 steps for students, 13 for teachers — the extra length is the point, not an oversight). `data-tour` was added to every remaining `NavItem` in `StudentSidebar.tsx` and `TeacherSidebar.tsx` (previously only `nav-teachers`/`nav-classes` had it). The dashboard-widget spotlighting from the prior pass (`next-up-card`, `academic-arc-widget`, `mentor-milestones-widget`) was dropped in favor of this sidebar-first structure — their mechanics got folded into the Dashboard step's own description instead — so the now-unused `data-tour` attributes were removed from those three widget files. `weekly-quests-widget` is the one dashboard-body element kept as a dedicated closing step, since "check back every Monday" is a habit worth pointing at directly rather than just describing in passing.

## Teacher payouts via Stripe Connect (15% platform commission)

Until now every class payment landed entirely in the platform's own Stripe account with no mechanism to pay teachers out at all. The user asked for the platform to take a commission per class and for teachers to be paid automatically; after weighing a manual/ledger-only alternative, **Stripe Connect** was chosen so Stripe handles bank-account collection, identity verification, and the actual money movement. Commission is a flat **15%** (teacher keeps 85%), recommended as a competitive middle ground against comparable 1:1 tutoring marketplaces (Preply/Italki ~15-20% vs. Wyzant/TutorMe 25-30%).

**Architectural decision: separate charges & transfers, not destination charges.** Two Stripe-supported patterns exist for splitting marketplace payments. Went with capturing exactly as today (pre-auth → accept → capture, completely untouched) plus a *separate* `stripe.transfers.create` call that only fires once a class reaches `"completed"` — from both the manual "Mark Complete" action (`completeClass` in `classes.actions.ts`) and the worker's automated poller (`markCompletedClasses` in `worker/src/complete-classes.ts`). Destination charges (`transfer_data.destination` on the PaymentIntent itself) were rejected because they'd move money to the teacher at capture time, well before a class actually happens — any subsequent refund would then need `reverse_transfer` against a connected-account balance that might already be withdrawn. Under the chosen model, `cancelClassCore`'s existing time-tiered refund logic needed **zero changes**: cancellation is only ever possible before a class completes, and a transfer only ever exists after that point, so a transfer to a teacher's connected account can never exist for a class that later gets cancelled/refunded — confirmed by walking the actual `cancelClassCore` code path, not just architecturally assumed.

**Schema** (migration `20260723211343_add_stripe_connect_payouts`): new `ConnectStatus` (`not_started`/`pending`/`restricted`/`active`) and `PayoutStatus` (`not_applicable`/`pending`/`transferred`/`failed`) enums; `User` gains `stripeConnectAccountId`/`connectStatus`/`connectChargesEnabled`/`connectPayoutsEnabled`/`connectDetailsSubmitted`/`connectUpdatedAt` (no separate Teacher model exists, so like `pricePerHour`/`teachingStyle` these live on `User` even though only ever populated for `role: teacher`); `Payment` gains `platformFeeAmount`/`teacherPayoutAmount`/`platformFeeRateBps` (computed once via `computeCommissionSplit()` at charge/capture time, never recomputed later so a future rate change can't silently reprice history) plus `payoutStatus`/`transferId`/`payoutAttemptedAt`/`payoutError` (mutated later, at completion time). Extended `Payment` in place rather than adding a sibling `Payout` model — one `Payment` is always at most one payout in this design, so a join would add nothing. The migration backfills existing rows to the flat 15% split but deliberately leaves `payoutStatus: "not_applicable"` (not `"pending"`) — retroactively transferring money for classes that happened before this feature existed, under a rate that didn't exist at the time, is a business decision for an explicit future action, not something a migration should trigger silently.

**`app/lib/payouts-utils.ts`** — pure `computeCommissionSplit(amount, feeRateBps = 1500)`, rounds from total cents first so the two halves always add back up to the original amount exactly (no off-by-a-cent drift). Takes an optional rate override so a future tier-based discount (e.g. tied to `TeacherGameProfile.mentorshipRank`, floated as an idea in this doc's product-vision phases) is a call-site change, not a rearchitecture — not implemented now, flat rate only.

**`app/lib/payouts.ts`** — `transferPayoutForClass(classId)` and `transferPendingPayoutsForTeacher(teacherId)`, both `"use server"` and shared between the Next app and the standalone `worker/` package the exact same way `gamification.ts` already is (confirmed the worker already cross-imports from `app/lib/*` successfully). Idempotency against the manual action and the worker's poller racing on the same class is handled via a claiming `prisma.payment.updateMany` (conditional on `payoutStatus` not already being a terminal state) *before* calling Stripe — only the caller whose update actually affects a row proceeds, rather than relying on the two independent triggers to coordinate. A teacher with no Connect account yet, or `connectPayoutsEnabled: false`, gets their payout parked at `payoutStatus: "pending"` instead of failing.

**Onboarding** (`app/lib/actions/payouts.actions.ts`): `startConnectOnboarding()` creates a Stripe Express account on first use (`stripe.accounts.create`) and always returns a fresh `stripe.accountLinks.create` URL — Account Links are single-use and expire in minutes, so this is never cached. `account.updated` (new case in `app/api/webhooks/stripe/route.ts`) derives `ConnectStatus` from `charges_enabled`/`payouts_enabled`/`details_submitted` and, on the transition to fully `active`, calls `transferPendingPayoutsForTeacher` to sweep everything that accrued while the teacher hadn't onboarded yet.

**UI**: new `/main/teacher/payouts` page (onboarding CTA/status + payout history table), a "Payouts" sidebar entry and matching welcome-tour step. `earnings/page.tsx`'s "Total Earned" changed from gross (`sum(amount)`) to net (`sum(teacherPayoutAmount)`) with a small-print platform-fee line — a visible behavior change from what shipped before this feature existed. Admin `payments/page.tsx` split its single (mislabeled — it was gross pass-through, not platform revenue) stat into Gross volume / Platform revenue / Teacher payouts, plus a payout-status column; admin teachers table gained a Connect-status badge via `fetchTeachersExtended`.

Added `payouts-utils.test.ts`, `payouts.test.ts`, `payouts.actions.test.ts` (new, 33 tests total across the feature) plus extensions to `classes.actions.test.ts` and `webhooks/stripe/route.test.ts`. Verified: `tsc --noEmit` clean, `pnpm lint` clean, `pnpm test` (425/425 passing), production `pnpm build` succeeds — `/main/teacher/payouts` compiles as a real route. Not run against a real Stripe test-mode Connect account or verified in a browser this session.
