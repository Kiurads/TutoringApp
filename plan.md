...
**Tour rebuilt on driver.js.** `welcome-tour-modal.tsx` no longer renders its own `<dialog>` carousel — it drives `driver.js` against real dashboard elements (`data-tour="nav-teachers"` / `nav-classes` on the sidebars, `data-tour="academic-arc-widget"` / `mentor-milestones-widget"` on the gamification cards). `allowClose` toggles per role: `true` for students (X button, overlay click, Escape all still dismiss it, same low-friction philosophy as before), `false` for teachers (none of those work — driver.js hides the close button and disables overlay/Escape dismissal entirely when `allowClose` is `false`, confirmed by reading the library source directly rather than assuming). A single global `onDestroyed` hook is the only place `finish()` is called from; the destination to navigate to (if any) rides on the last step's `data.destination` field rather than being threaded through separately. An `isCleanupDestroy` flag guards against React StrictMode's dev-mode double-invoke (mount→cleanup→mount) from firing the completion side effect prematurely — effect cleanup also calls `driverObj.destroy()`, which would otherwise look identical to a real dismissal.
...
This reorders the teacher's first-login sequence from what was shipped originally: middleware now forces brand-new teachers straight to `/main/teacher/onboarding` (preferences) *before* they ever reach the dashboard, and only once `teachingStyle` is set do they land on the dashboard, where the (still-mandatory, still-unskippable) driver.js welcome tour plays. Its last step no longer offers a "Set up my preferences" CTA for teachers — there's nothing left to set up at that point — it just says "Let's get started" and closes. Students are unaffected: their tour is still fully skippable at every step, and their preferences page remains freely revisitable with no middleware gate.
