...
3. **`RegularClass` has zero UI or actions** — if asked to build recurring-class booking, this is greenfield; decide early whether it reuses `classes.actions.ts` patterns or needs its own action file, and whether it needs its own status lifecycle.
