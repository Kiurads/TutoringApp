...
import { getRankName, getRankProgress } from "@/app/lib/gamification-utils";

export default async function MentorMilestonesWidget({
	userEmail,
}: {
	userEmail: string;
}) {
	...
	const profile = await prisma.teacherGameProfile.findUnique({
		where: { userId: user.id },
	});

	const sparks = profile?.reputationSparks ?? 0;
	const rank = profile?.mentorshipRank ?? 0;
	...
	return (
		<div className="card bg-base-200 shadow-lg" data-tour="mentor-milestones-widget">
			<div className="card-body gap-4">
				...
				<div className="flex items-center gap-2">
					<i className="fa-solid fa-star text-warning text-lg"></i>
				...
				{!isMaxRank && (
					<div className="flex flex-col gap-1">
						<div className="flex justify-between text-xs text-base-content/50">
							<span>{progress.current} / {progress.next} sparks to next rank</span>
				...
				{isMaxRank && (
					<p className="text-xs text-success font-medium">
						<i className="fa-solid fa-check-circle mr-1"></i>Max rank reached!
					</p>
				)}
				...
				{recentBadges.length > 0 && (
					<div className="flex flex-col gap-2">
						<p className="text-xs text-base-content/50">Expertise seals</p>
						<div className="flex gap-2 flex-wrap">
							{recentBadges.map((ub) => (
								<div key={ub.id} className="tooltip" data-tip={ub.badge.name}>
									<div className="badge badge-outline gap-1 py-3">
										<i className={`fa-solid ${ub.badge.iconKey} text-accent`}></i>
				...
				{recentBadges.length === 0 && (
					<p className="text-xs text-base-content/40">
						Accept classes and get reviews to earn seals.
					</p>
				)}
			</div>
		</div>
