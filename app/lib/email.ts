...
const FROM_EMAIL =
	process.env.RESEND_FROM_EMAIL || "Ponte <onboarding@resend.dev>";
...
export async function sendVerificationEmail(to: string, verifyUrl: string) {
	await sendEmail({
		to,
		subject: "Verify your email address for Ponte",
		html: `
			<p>Welcome to Ponte!</p>
...
export async function sendPasswordResetEmail(to: string, resetUrl: string) {
	await sendEmail({
		to,
		subject: "Reset your password for Ponte",
		html: `
			<p>We received a request to reset your Ponte account password.</p>
...
// dispute isn't actionable from inside Ponte at all, so email
...
export async function sendDisputeAlertEmail(
	...
	await sendEmail({
		...
		html: `
			<p>A payment dispute was opened on a Stripe charge linked to Ponte.</p>
			<ul>
...
export async function sendPaymentIssueAlertEmail(
	...
	await sendEmail({
		...
		subject,
		html: `
			<p>A Stripe payment intent linked to Ponte needs manual review.</p>
			<ul>
